...
class PreferContainsMethodRule extends DartLintRule {
  ...
  const PreferContainsMethodRule()
    : super(
        code: const LintCode(
          name: ruleName,
          problemMessage: 'Suggests using .contains instead of .indexOf when checking for the presence of an element.',
        ),
      );
...
class _PreferContainsMethodFix extends DartFix {
  ...
  void run(
    ...
    context.registry.addMethodInvocation((node) {
      ...
      final changeBuilder = reporter.createChangeBuilder(message: 'Replace with contains', priority: 80);
