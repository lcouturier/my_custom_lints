...
class AvoidBannedTypeRule extends BaseLintRule<AvoidBannedTypeParameters> {
  AvoidBannedTypeRule._(super.rule);

  factory AvoidBannedTypeRule.createRule(CustomLintConfigs configs) {
    final rule = RuleConfig(
      configs: configs,
      name: 'avoid_banned_type',
      paramsParser: AvoidBannedTypeParameters.fromJson,
      problemMessage: (value) => '{0}',
    );

    return AvoidBannedTypeRule._(rule);
  }
...
class AvoidBannedTypeParameters {
  ...
  AvoidBannedTypeParameters._(this.entries);
}
