...
class _AvoidUnnecessaryNullAssertionFix extends DartFix {
  ...
  void run(
    ...
    context.registry.addIfStatementNullAssertion((condition, node, statements) {
      ...
      final changeBuilder = reporter.createChangeBuilder(message: 'if...case notation', priority: 80);

      final variable = switch (node) {
        PrefixedIdentifier() => node,
        _ => node as SimpleIdentifier,
      };

      changeBuilder.addDartFileEdit((builder) {
        if (variable is PrefixedIdentifier) {
          builder.addSimpleReplacement(
            range.node(condition),
            '${variable.name} case final ${variable.identifier.name}?',
          );
        } else {
        ...
        for (final expression in statements.where((e) => e.toSource().contains('${variable.name}!'))) {
          if (variable is PrefixedIdentifier) {
            ...
            builder.addSimpleReplacement(
              range.node(expression),
              expression.toSource().replaceAll('${variable.name}!', variable.name),
            );
          }
        }
      });
