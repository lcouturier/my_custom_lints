  ...
  void run(
    ...
    context.registry.addEnumDeclaration((node) {
      if (analysisError.sourceRange.covers(node.beginToken.next!.sourceRange)) {
        final changeBuilder = reporter.createChangeBuilder(message: 'Remove Enum prefix', priority: 80);
        ...
        if (analysisError.data != null) {
          ...
          final changeBuilder = reporter.createChangeBuilder(message: 'Remove $prefix prefix', priority: 80);
          // ignore: cascade_invocations
          changeBuilder.addDartFileEdit((builder) {
            builder.addSimpleReplacement(value.sourceRange, value.name.lexeme.removePrefix(prefix));
          });
        }
      }
    });
